...
puts 'Initializers script is being loaded'


return unless Rails.env.test?

puts 'Starting server for Cypress tests'

CypressRails.hooks.before_server_start do
  # Called once, before either the transaction or the server is started
  cat1 = Category.find_or_create_by! name: 'Evergreens'
  puts "Category created: #{cat1.name}, ID: #{cat1.id}"
  ...
  puts "Product 1 created: #{product1.name}, ID: #{product1.id}"
  ...
  puts "Product 2 created: #{product2.name}, ID: #{product2.id}"
end
